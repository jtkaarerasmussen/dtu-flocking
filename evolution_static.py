...
class StaticEvolutionSimulation:
    """
    ...
    def __init__(self, world_size=2.0, num_agents=640, density=2.77e-2,
                 pg=1.0, ps=0.0, g_evol=True, s_evol=True , g_mu=0.1, s_mu=0.1, g_sigma=0.1,s_sigma=0.1, omega_gc=4.0, omega_sc=4.0,
    ...
    def _initialize_population(self):
        ...
        for i in range(self.num_agents):
            if self.g_evol:
                ...
                wg = max(0,np.random.normal(self.g_mu,self.g_sigma))

            if self.s_evol:
                ws = np.random.rand()*2
            else:
                ws = max(0,np.random.normal(self.s_mu,self.s_sigma))
            individual = {
                'w_g': wg,
                'w_s': ws,
                'fitness': 0.0,
    ...
    def mutate_population(self, population):
        ...
        for individual in population:
            ...
            if self.g_evol:
                ...
                individual['w_g'] = max(0.0, np.random.normal(self.g_mu, self.g_sigma))
            ...
            if self.s_evol:
                ...
                individual['w_s'] = max(0.0, np.random.normal(self.s_mu, self.s_sigma))
...
def run_evolution_from_params(params_dict_or_file, num_generations=None, start_fresh=False, checkpoint_dir=None):
    ...
    # Remove num_generations from params before passing to constructor
    simulation_params = {k: v for k, v in params.items() if k != 'num_generations'}
    
    # Create simulation instance
    sim = StaticEvolutionSimulation(**simulation_params)
